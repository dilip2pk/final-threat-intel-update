...
<body style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;background:#f5f5f5;">
  ...
  const [advisoryTemplate, setAdvisoryTemplateRaw] = useState(() => getCached<any>("advisory_template") || {
  ...
  const setAdvisoryTemplate = (valOrFn: any) => {
    setAdvisoryTemplateRaw((prev: any) => {
      const next = typeof valOrFn === "function" ? valOrFn(prev) : valOrFn;
      setLocalCache("advisory_template", next);
      return next;
    });
  };
  ...
  const [watchlistNotify, setWatchlistNotifyRaw] = useState(() => getCached<any>("watchlist_notifications") || {
    enabled: false, recipients: "", frequency: "daily", method: "email",
    emailTemplate: defaultWatchlistTemplate,
  });
  const setWatchlistNotify = (valOrFn: any) => {
    setWatchlistNotifyRaw((prev: any) => {
      const next = typeof valOrFn === "function" ? valOrFn(prev) : valOrFn;
      setLocalCache("watchlist_notifications", next);
      return next;
    });
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "advisory-template": {
        const templateSubTab = searchParams.get("subtab") || "advisory";
        const setTemplateSubTab = (sub: string) => {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-border pb-2">
              {[
                { id: "advisory", label: "Advisory Template", icon: FileText },
                { id: "alert", label: "Alert Template", icon: Bell },
              ].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setTemplateSubTab(tab.id)}
                  className={cn(
                    "flex items-center gap-1.5 px-3 py-1.5 rounded-t-lg text-sm font-medium transition-colors",
                    templateSubTab === tab.id
            ...
            {templateSubTab === "alert" ? (
              ...
              <SectionCard title="Alert Notification Template" icon={Bell} description="This template is used for real-time alert notifications sent via webhooks (Slack, Teams, Discord) and plain-text email alerts when new threats match your alert rules. It is NOT the same as the Advisory Template — this is for quick, automated notifications.">
                ...
                <FieldGroup label="Footer Text" description="Shown at the bottom of alert emails. Editable to match your organization branding.">
                  <Input value={general.alertFooterText || ""} onChange={e => setGeneral(g => ({ ...g, alertFooterText: e.target.value }))} placeholder="Generated by Threat Intelligence Platform — Alert Monitoring" />
                </FieldGroup>
              </SectionCard>

              <SectionCard title="Preview" icon={Eye} description="Live preview with sample alert data.">
                <div className="border border-border rounded-xl overflow-hidden bg-muted/30 p-4 space-y-3">
                  ...
                  <div className="border-t border-border pt-2 mt-2">
                    <p className="text-xs text-muted-foreground italic">{general.alertFooterText || "Generated by Threat Intelligence Platform — Alert Monitoring"}</p>
                  </div>
                </div>
              </SectionCard>
